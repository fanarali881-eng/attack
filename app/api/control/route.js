  ...
  try {
    ...
    const getCommand = () => {
      if (action === 'setup') {
        ...
        const d = duration || 5;
        const killCmd = 'kill $(pgrep -f visit.py) 2>/dev/null; kill $(pgrep -f chrome) 2>/dev/null; rm -f /root/visit_status.json;';
        if (proxies && proxies.length > 0) {
          const proxyB64 = Buffer.from(JSON.stringify(proxies)).toString('base64');
          return `${killCmd} echo "${proxyB64}" | base64 -d > /root/proxies.json && bash -c "nohup python3 /root/visit.py '${url}' ${v} ${d} /root/proxies.json > /root/visit.log 2>&1 &" && echo "Started"`;
        }
        return `${killCmd} bash -c "nohup python3 /root/visit.py '${url}' ${v} ${d} > /root/visit.log 2>&1 &" && echo "Started"`;
      } else if (action === 'stop') {
